...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=str, default="outputs/final-model/sample.png")
...
def load_or_build_corr_state(
    ...
    with torch.no_grad():
        f_c_dino = dino_extractor([content_pil], grid_hw)[0]
        f_r_dino = dino_extractor([reference_pil], grid_hw)[0]
        ...
        raw = compute_corr_state(
            ...
            top_m_regions=int(ckpt_args.get("top_m_regions", 2)),
            top_k_sparse=int(ckpt_args.get("top_k_sparse", 16)),
            lambda_vlm_token=float(ckpt_args.get("lambda_vlm_token", 0.15)),
            lambda_vlm_region=float(ckpt_args.get("lambda_vlm_region", 0.25)),
            lambda_region_token=float(ckpt_args.get("lambda_region_token", 0.20)),
            tau_sparse=float(ckpt_args.get("tau_sparse", 0.07)),
            tau_region=float(ckpt_args.get("tau_region", 0.10)),
            lambda_xy=float(ckpt_args.get("lambda_xy", 0.05)),
            kmeans_iters=int(ckpt_args.get("kmeans_iters", 10)),
...
def main() -> None:
    ...
    dino_model = args.dino_model or ckpt_args.get("dino_model", "model/dinov2-large")
    dino_extractor = DinoV2FeatureExtractor(dino_model, device=device, dtype=dtype)
    ...
    use_cig = (
        ...
        and bool(ckpt_args.get("use_cig", True))
        and not bool(ckpt_args.get("disable_cig", False))
    )
    use_depth = bool(ckpt_args.get("use_depth", True)) and not bool(ckpt_args.get("disable_depth", False))
    use_segmentation = (
        (args.use_segmentation or bool(ckpt_args.get("use_segmentation", False)))
        and not args.disable_segmentation
        and not bool(ckpt_args.get("disable_segmentation", False))
    )
    use_panoptic = (
        (args.use_panoptic or bool(ckpt_args.get("use_panoptic", False)))
        and not args.disable_panoptic
        and not bool(ckpt_args.get("disable_panoptic", False))
    )
    depth_extractor = None
    if use_cig and use_depth and not args.disable_depth:
        depth_extractor = DepthAnythingV2FeatureExtractor(
            args.depth_model or ckpt_args.get("depth_model", "model/depth-anything-v2-base"),
            device=device,
            dtype=dtype,
        )
    segmentation_extractor = None
    if use_cig and use_segmentation:
        segmentation_extractor = SegFormerSemanticStructureExtractor(
            args.segmentation_model or ckpt_args.get("segmentation_model", "model/segformer-b0-ade"),
            device=device,
            dtype=dtype,
        )
    panoptic_extractor = None
    if use_cig and use_panoptic:
        panoptic_extractor = Mask2FormerPanopticStructureExtractor(
            args.panoptic_model or ckpt_args.get("panoptic_model", "model/mask2former-swin-small-coco-panoptic"),
            device=device,
            dtype=dtype,
        )
    ...
    inject_corr_reference_attention(
        ...
        num_intrinsic_masks=int(ckpt_args.get("cig_num_masks", 6)),
        max_key_bias=float(ckpt_args.get("cig_max_key_bias", 0.8)),
        max_anchor_scale=float(ckpt_args.get("cig_max_anchor_scale", 0.15)),
        max_local_protect=float(ckpt_args.get("cig_max_local_protect", 0.30)),
        max_region_protect=float(ckpt_args.get("cig_max_region_protect", 0.12)),
    )
    add_sd3_transformer_lora(
        transformer,
    ...
    intrinsic_prior_tokenizer = IntrinsicPriorTokenizer(
        ...
        hidden_dim=int(ckpt_args.get("cig_hidden_dim", 0)) or hidden_size,
        num_mask_types=int(ckpt_args.get("cig_num_masks", 6)),
    ).to(device=device, dtype=dtype)
    ...
    load_corr_reference_processor_state_dict(transformer, checkpoint.get("corr_reference_processors"), strict=False)
    transformer.to(device=device, dtype=dtype)

    vae.eval()
    transformer.eval()
    reference_adapter.eval()
    intrinsic_prior_tokenizer.eval()
    ...
    with torch.no_grad():
        ...
        if use_cig:
            depth_map = depth_extractor([content_pil], content.shape[-2:], dtype=dtype) if depth_extractor is not None else None
            semantic_state = (
                segmentation_extractor([content_pil], content.shape[-2:], dtype=dtype)
                if segmentation_extractor is not None
                else None
            )
            panoptic_state = (
                panoptic_extractor([content_pil], content.shape[-2:], dtype=dtype)
            ...
            xy = xy_grid(corr_state.grid_hw, device=device, dtype=dtype).unsqueeze(0)
