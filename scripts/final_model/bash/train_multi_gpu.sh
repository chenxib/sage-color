...
if [[ "${USE_DEPTH}" == "0" ]]; then
  train_args+=(--no-use_depth)
fi

if [[ "${DISABLE_DEPTH}" == "1" ]]; then
  train_args+=(--disable_depth)
fi

if [[ "${USE_SEGMENTATION}" == "1" ]]; then
  train_args+=(--use_segmentation)
fi

if [[ "${DISABLE_SEGMENTATION}" == "1" ]]; then
  train_args+=(--disable_segmentation)
fi

if [[ "${USE_PANOPTIC}" == "1" ]]; then
  train_args+=(--use_panoptic)
fi

if [[ "${DISABLE_PANOPTIC}" == "1" ]]; then
  train_args+=(--disable_panoptic)
fi

if [[ "${DISABLE_CIG}" == "1" ]]; then
  train_args+=(--disable_cig)
fi

if [[ "${USE_COLOR_LOSS}" != "1" ]]; then
  train_args+=(--no-use_color_loss)
fi
