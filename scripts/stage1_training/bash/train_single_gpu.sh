...
#   OUTPUT_DIR=outputs/stage1-online-smoke \
...
cmd=(
  "${PYTHON_BIN}" scripts/stage1_training/train.py
...
if [[ -n "${INIT_FROM_STAGE1_BASE_CHECKPOINT}" ]]; then
  cmd+=(--init_from_stage1_base_checkpoint "${INIT_FROM_STAGE1_BASE_CHECKPOINT}")
fi
